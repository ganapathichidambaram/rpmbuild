...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            let name = '';
            let version = '';
            for (var line of data.toString().replace(/\t/g, '').split('\n')) {
            ...
            yield exec.exec(`rm -rf  /github/home/rpmbuild/SRPMS/*`);
            yield exec.exec(`rm -rf /github/home/rpmbuild/RPMS/*`);
            // setup rpm tree
            ...
            if (myToken)
                yield exec.exec(`curl -H "Authorization: token ${myToken}" -L --output tmp.tar.gz https://api.github.com/repos/${owner}/${repo}/tarball/${ref}`);
            else
            ...
            yield exec.exec(`mkdir -p ${name}`);
            // Extract source code 
            yield exec.exec(`tar xvf tmp.tar.gz -C ${name} --strip-components 1`);
            // Create Source tar.gz file 
            yield exec.exec(`tar -czvf ${name}-${version}.tar.gz ${name}`);
            ...
            try {
                yield exec.exec(`rpmbuild -ba /github/workspace/${specFile}`);
            }
            catch (err) {
            ...
            yield exec.exec(`rm -rf  rpmbuild/SRPMS/*`);
            yield exec.exec(`rm -rf rpmbuild/RPMS/*`);
            ...
            core.setOutput("version", `${version}`); // Content-type for Upload
        }
        catch (error) {
